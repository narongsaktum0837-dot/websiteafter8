...
function renderCart(){
  ...
  $("cartBox").innerHTML=`<div class="layout"><div>${cart.map((x,i)=>`<div class="cartrow"><div class="pic"><img src="${x.image ? x.image.replace(/^\/?assets\//,"") : "product-tee.png"}"></div><div><b>${x.name}</b><div class="muted">${x.size}</div><div>${money(x.price)} × ${x.qty}</div></div><button class="textbtn" onclick="removeItem(${i})">×</button></div>`).join("")}</div><aside class="summary"><div class="sum"><span>Subtotal</span><b>${money(sub)}</b></div><div class="sum"><span>Shipping</span><span>${ship?money(ship):"FREE"}</span></div><div class="sum"><b>Total</b><b>${money(sub+ship)}</b></div><br><button class="btn" style="width:100%" onclick="go('checkout')">Checkout</button></aside></div>`;
}
